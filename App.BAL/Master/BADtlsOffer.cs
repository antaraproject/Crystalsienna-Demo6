...
{
    public class BADtlsOffer : IDtlsOffer
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_DTLS_OFFER", ref ddl, "DTLS_OFFER_KEY", "OFFER_NAME", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_OFFER", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityDtlsOffer oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@DTLS_OFFER_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_OFFER_KEY;
                para[vCount] = new SqlParameter("@OFFER_NAME", SqlDbType.VarChar, 300);
                para[vCount++].Value = oEntity.OFFER_NAME;
                para[vCount] = new SqlParameter("@OFFER_PER", SqlDbType.VarChar, 100);
                para[vCount++].Value = oEntity.OFFER_PER;
                para[vCount] = new SqlParameter("@OFFER_IMG", SqlDbType.VarChar, 9000);
                para[vCount++].Value = oEntity.OFFER_IMG;
                para[vCount] = new SqlParameter("@OFFER_NOTE", SqlDbType.VarChar, 100);
                para[vCount++].Value = oEntity.OFFER_NOTE;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_OFFER", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_OFFER_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityDtlsOffer oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_DTLS_OFFER", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_OFFER_KEY", ref pMsg), typeof(T2));
