...
{
    public class BADtlsSelfie : IDtlsSelfie
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public void BindDdl(ref DropDownList ddl, ref string pMsg, string pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (CommonOp oCommonOp = new CommonOp())
                {
                    ...
                    oCommonOp.BindDropDownList("SP_GET_DTLS_SELFIE", ref ddl, "DTLS_SELFIE_KEY", "SELFIE_DESC", ref pMsg, pAccYr, pCompany_key, para);
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_SELFIE", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityDtlsSelfie oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@DTLS_SELFIE_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_SELFIE_KEY;
                para[vCount] = new SqlParameter("@SELFIE_DESC", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.SELFIE_DESC;
                para[vCount] = new SqlParameter("@SELFIE_IMG", SqlDbType.VarChar, 100);
                para[vCount++].Value = oEntity.SELFIE_IMG;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_SELFIE", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_SELFIE_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityDtlsSelfie oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_DTLS_SELFIE", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_SELFIE_KEY", ref pMsg), typeof(T2));
