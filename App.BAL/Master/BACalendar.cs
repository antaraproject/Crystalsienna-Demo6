...
{
    public class BACalendar : ICalendar
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public void BindDdl(ref DropDownList ddl, Int32 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_DTLS_CALENDER", ref ddl, "MAST_PAGE_KEY", "PAGE_NAME", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_CALENDER", CommandType.StoredProcedure))
                {
        ...
        public Byte SaveChanges<T1, T2>(String pMode, EntityCalendar oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@DTLS_CALENDER_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_CALENDER_KEY;
                para[vCount] = new SqlParameter("@DAY_NAME", SqlDbType.NVarChar, 100);
                para[vCount++].Value = oEntity.DAY_NAME;
                para[vCount] = new SqlParameter("@START_TIME", SqlDbType.NVarChar, 50);
                para[vCount++].Value = oEntity.START_TIME;
                para[vCount] = new SqlParameter("@END_TIME", SqlDbType.NVarChar, 50);
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_CALENDER", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_CALENDER_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
