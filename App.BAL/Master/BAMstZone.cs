...
{
   public class BAMstZone :IMstZone
   {
       Int16 vCount = 0;
       DataTable dt = null;
       ...
       public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
       {
           try
           {
               ...
               {
                   oCommonOp.BindDropDownList("SP_GET_MAST_ZONE", ref ddl, "MAST_ZONE_KEY", "ZONE_NAME", ref pMsg, pAccYr, pCompany_key, para);
               }
           }
           catch (Exception ex)
           {
       ...
       public void BindLB(ref ListBox ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
       {
           try
           {
               ...
               using (CommonOp oCommonOp = new CommonOp())
               {
                   oCommonOp.BindListBox("SP_GET_MAST_ZONE", ref ddl, "MAST_ZONE_KEY", "ZONE_NAME", ref pMsg, pAccYr, pCompany_key, para);
               }
           }
           catch (Exception ex)
           {
       ...
       public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
       {
           try
           {
               ...
               using (DBContext oDBC = new DBContext("SP_GET_MAST_ZONE", CommandType.StoredProcedure))
               {
       ...
       public byte SaveChanges<T1, T2>(String pMode, EntityMstZone oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
       {
           Byte vRef = 0;
           try
           {
               ...
               para[vCount++].Value = 0;
               para[vCount] = new SqlParameter("@MAST_ZONE_KEY", SqlDbType.Int);
               para[vCount].Direction = ParameterDirection.InputOutput;
               para[vCount++].Value = oEntity.MAST_ZONE_KEY;
               para[vCount] = new SqlParameter("@ZONE_CODE", SqlDbType.VarChar, 10);
               para[vCount++].Value = oEntity.ZONE_CODE;
               para[vCount] = new SqlParameter("@ZONE_DESC", SqlDbType.VarChar, 250);
               para[vCount++].Value = oEntity.ZONE_DESC;
               ...
               para[vCount++].Value = oEntity.TAG_DELETE;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_MAST_ZONE", CommandType.StoredProcedure))
               {
                   oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                   vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                   pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_ZONE_KEY", ref pMsg), typeof(T2));
               }
           }
           catch (Exception ex)
           {
