...
{
    public class BADtlsEvent : IDtlsEvent
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_DTLS_EVENTS", ref ddl, "DTLS_EVENTS_KEY", "EVENTS_TITLE", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_EVENTS", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityDtlsEvent oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@DTLS_EVENTS_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_EVENTS_KEY;
                para[vCount] = new SqlParameter("@EVENTS_TITLE", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.EVENTS_TITLE;
                para[vCount] = new SqlParameter("@EVENTS_DESC", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.EVENTS_DESC;
                para[vCount] = new SqlParameter("@EVENT_DATE", SqlDbType.SmallDateTime);
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_EVENTS", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_EVENTS_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityDtlsEvent oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_DTLS_EVENTS", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_EVENTS_KEY", ref pMsg), typeof(T2));
