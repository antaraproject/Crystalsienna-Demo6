...
{
    public class BADtlsDiary : IDtlsDiary
    {
        Int16 vCount = 0;
        DataTable dt = null;
        DataSet ds = null;
        ...
        public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_MAST_TYPE", ref ddl, "MAST_TYPE_KEY", "TYPE_NAME", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_DIARY", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityDtlsDiary oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount] = new SqlParameter("@DTLS_DIARY_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_DIARY_KEY;
                para[vCount] = new SqlParameter("@DIARY_HEADING", SqlDbType.VarChar, 200);
                para[vCount++].Value = oEntity.DIARY_HEADING;
                para[vCount] = new SqlParameter("@DIARY_DATE", SqlDbType.SmallDateTime);
                para[vCount++].Value = oEntity.DIARY_DATE;
                para[vCount] = new SqlParameter("@SHORT_DESC", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.SHORT_DESC;
                para[vCount] = new SqlParameter("@LONG_DESC", SqlDbType.VarChar, 9000);
                para[vCount++].Value = oEntity.LONG_DESC;
                para[vCount] = new SqlParameter("@DIARY_IMG", SqlDbType.VarChar, 9000);
                para[vCount++].Value = oEntity.DIARY_IMG;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_DIARY", CommandType.StoredProcedure))
                {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityDtlsDiary oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_DTLS_DIARY", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_DIARY_KEY", ref pMsg), typeof(T2));
