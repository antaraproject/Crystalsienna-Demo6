   ...
   public class BARoom : IRoom
    {
        ...
        public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_MAST_ROOM", ref ddl, "MAST_ROOM_KEY", "ROOM_HEADING", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_MAST_ROOM", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityRoom oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@MAST_ROOM_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.MAST_ROOM_KEY;
                para[vCount] = new SqlParameter("@ROOM_HEADING", SqlDbType.VarChar, 200);
                para[vCount++].Value = oEntity.ROOM_HEADING;
                para[vCount] = new SqlParameter("@ROOM_DETAILS", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.ROOM_DETAILS;
                para[vCount] = new SqlParameter("@ROOM_IMG_NAME", SqlDbType.VarChar, 500);
                para[vCount++].Value = oEntity.ROOM_IMG_NAME;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_MAST_ROOM", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_ROOM_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityRoom oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_MAST_ROOM", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_ROOM_KEY", ref pMsg), typeof(T2));
