...
{   
    public class BAMstEventType : IMstEventType
    {
        Int16 vCount = 0;
        DataTable dt = null;
        ...
        public void BindDdl(ref DropDownList ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                {
                    oCommonOp.BindDropDownList("SP_GET_MAST_EVENT_TYPE", ref ddl, "MAST_EVENT_TYPE_KEY", "EVENT_TYPE_DESC", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public void BindLB(ref ListBox ddl, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (CommonOp oCommonOp = new CommonOp())
                {
                    oCommonOp.BindListBox("SP_GET_MAST_EVENT_TYPE", ref ddl, "MAST_EVENT_TYPE_KEY", "EVENT_TYPE_DESC", ref pMsg, pAccYr, pCompany_key, para);
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_MAST_EVENT_TYPE", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityMstEventType oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@MAST_EVENT_TYPE_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.MAST_EVENT_TYPE_KEY;
                para[vCount] = new SqlParameter("@EVENT_TYPE_CODE", SqlDbType.VarChar, 10);
                para[vCount++].Value = oEntity.EVENT_TYPE_CODE;
                para[vCount] = new SqlParameter("@EVENT_TYPE_DESC", SqlDbType.VarChar, 250);
                para[vCount++].Value = oEntity.EVENT_TYPE_DESC;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_MAST_EVENT_TYPE", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_EVENT_TYPE_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
