...
namespace App.BAL.Master
{
    public class BARoster : IRoster
    {
        ...
        public Byte SaveChanges<T1, T2>(String pMode, EntityRoster oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_HEAD_SITE_SETTING", CommandType.StoredProcedure))
                {
        ...
        public Byte SaveLink<T1, T2>(String pMode, EntityRoster oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = 0;
                para[vCount] = new SqlParameter("@MAST_ROSTER_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.MAST_ROSTER_KEY;
                para[vCount] = new SqlParameter("@ROSTER_DATE", SqlDbType.DateTime);
                para[vCount++].Value = oEntity.ROSTER_DATE;
                para[vCount] = new SqlParameter("@MAST_TIME_KEY", SqlDbType.Int);
                para[vCount++].Value = oEntity.MAST_TIME_KEY;
                ...
                para[vCount] = new SqlParameter("@COMPANY_KEY", SqlDbType.Int);
                para[vCount++].Value = oEntity.COMPANY_KEY;
                using (DBContext oDBC = new DBContext("SP_SAVE_MAST_ROSTER", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@MAST_ROSTER_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public byte Delete<T>(String pMode, T pValue, ref T pKey, ref String pMsg, Int32 pUser, String pAccYr, Int16? pCompany_key)
        {
            ...
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_MAST_ROSTER", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
