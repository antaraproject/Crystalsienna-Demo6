...
{
    public class BADtlsGallery : IDtlsGallery
    {
        Int16 vCount = 0;
        DataTable dt = null;
        DataSet ds = null;
        ...
        public DataTable Get<T>(String pMode, T pKey, String pSEARCH_TEXT, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_GET_DTLS_GALLERY", CommandType.StoredProcedure))
                {
        ...
        public byte SaveChanges<T1, T2>(String pMode, EntityDtlsGallery oEntity, T1 pValue, ref T2 pKey, ref String pMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                para[vCount] = new SqlParameter("@DTLS_GALLERY_KEY", SqlDbType.Int);
                para[vCount].Direction = ParameterDirection.InputOutput;
                para[vCount++].Value = oEntity.DTLS_GALLERY_KEY;
                para[vCount] = new SqlParameter("@MAST_TYPE_KEY", SqlDbType.Int);
                para[vCount++].Value = oEntity.MAST_TYPE_KEY;
                para[vCount] = new SqlParameter("@TYPE_DESC", SqlDbType.VarChar, 100);
                para[vCount++].Value = oEntity.TYPE_DESC;
                para[vCount] = new SqlParameter("@GALLERY_IMG", SqlDbType.VarChar, 100);
                para[vCount++].Value = oEntity.GALLERY_IMG;
                ...
                using (DBContext oDBC = new DBContext("SP_SAVE_DTLS_GALLERY", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_GALLERY_KEY", ref pMsg), typeof(T2));
                }
            }
            catch (Exception ex)
            {
        ...
        public Byte SaveDelete<T1, T2>(String pMode, EntityDtlsGallery oEntity, T1 pValue, ref T2 pKey, ref String pMsg, ref String pDelMsg, String pAccYr, Int16? pCompany_key)
        {
            Byte vRef = 0;
            try
            {
                ...
                using (DBContext oDBC = new DBContext("SP_DELETE_DTLS_GALLERY", CommandType.StoredProcedure))
                {
                    oDBC.ExecuteNonQuery(pAccYr, pCompany_key, para, ref pMsg);
                    vRef = Convert.ToByte(oDBC.GetParameterValue("@RETURN_KEY", ref pMsg));
                    pKey = (T2)Convert.ChangeType(oDBC.GetParameterValue("@DTLS_GALLERY_KEY", ref pMsg), typeof(T2));
